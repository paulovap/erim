...
announced_mechanisms(#xmlel{ns = ?NS_XMPP, name = 'features'} = El) ->
    case erim_xml:get_element(El, ?NS_SASL, 'mechanisms') of
        undefined  -> [];
...
selected_mechanism(Mechanism) ->
    ...
    erim_xml:set_attribute(El, <<"mechanism">>, Mechanism).
...
selected_mechanism(Mechanism, "") ->
    ...
    erim_xml:set_cdata(El, "=");
selected_mechanism(Mechanism, Initial_Response) ->
    El = selected_mechanism(Mechanism),
    erim_xml:set_cdata(El, base64:encode_to_string(Initial_Response)).
...
response(Response_Data) ->
    ...
    erim_xml:set_cdata(El, base64:encode_to_string(Response_Data)).

%% @spec () -> Abort
%%     Abort = erim_xml:xmlel()
